...
{
    public abstract class LyricsProviderTestsBase : InfoProviderTestsBase
    {
        ...
        {
            Assert.Throws<Exception>(delegate { Provider.GetInfo(Tag, InfoProviderInputs.Default); });
            Assert.Throws<Exception>(delegate { Provider.GetInfo(InfoProviderInputs.Default); });
        }
    }
}